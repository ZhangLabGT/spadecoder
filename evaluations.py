...
def eval_deconv(gnd_truth,deconv_res):
    ### evaluation of deconvolution results with spearmans correlation
    ...
    cell_types = list(set(gnd_truth.columns).union(set(deconv_res.index)))
    
    if set(cell_types) != set(gnd_truth.columns):
        for entry in cell_types:
            if entry not in list(gnd_truth.columns):
                gnd_truth[entry] = 0.0
    

    if set(cell_types) != set(deconv_res.index):
        for entry in cell_types:
            if entry not in list(deconv_res.index):
                deconv_res.loc[entry,] = 0.0


    gnd_truth = gnd_truth[cell_types].copy()
    deconv_res = deconv_res.loc[cell_types,:].copy()
    ...
    rmse = np.sqrt(((gnd_truth_norm - deconv_res) ** 2).sum())
    ...
    return [rmse, ari, purity,avg_corr_sp,avg_corr_pe], [correlations_sp,correlations_pe]      # , spear_correl




def eval_deconv2(gnd_truth,deconv_res):
    ### evaluation of deconvolution results with spearmans correlation
    # make sure gnd_truth and deconv_res have the same order of cell-names, even if the names differ 
...
def eval_deconv3(gnd_truth,deconv_res):
    ...
    # print([rmse, ari, purity,avg_corr], correlations)
    return [orig_rmse, new_rmse, ari, purity,avg_corr_sp,avg_corr_pe, avg_jsd], [correlations_sp,correlations_pe,cell_type_jsd]      # , spear_correl
