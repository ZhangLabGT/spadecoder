# !!!! CHANGES !!!! 
# every 10th spots 
# also only saved the last spot debug variables - later removed all of them 
...
def run_adam_softmax_optimization(Bf,  X1, # X2,  
    ...
    for iteration in range(max_iter_adam):
        ...
        # cost, term1,  l2_term, res1_term = compute_cost_function(V, res1,alpha, X1, #X2, 
    ...
    cell_wts = res1 @ V
    # print(res1.shape, V.shape, cell_wts.shape)
    # multiple V and res1 to get cell-type proportions
...
def get_adam_softmax_solution_perslice(#Palign,
    ...
    for spt_num in range(n_spots_st1): # range(0, n_spots_st1, 5):  
        ...
        if adata_bulk_init is not None:
            ...
            # for non-doublet spt_num was a str, changed here since in doublet simulator I simulated it as an integer
            V_init = np.array(adata_bulk_init.loc[all_ctypes,spt_num]).reshape(-1,1)
            V_init = torch.tensor(V_init,dtype=torch.float32,requires_grad=False).to(device)

        # deconv_op,res1, alpha,  debug_vars_list = run_adam_softmax_optimization(Bscrna, X1, #X2, 
    ...
    return deconv_df, cell_wts_df # ,res1, alpha, [debug_vars_list] #debug_vars_list_spots
...
def adam_softmax_solution_perslice_singlecellref_wrapper(adata_st1, #adata_st2, 
    ...
    # print("Bscrna")
    # print(Bscrna.shape)
    # deconv_st1 ,res1_1,alpha,  debug_vars1 = get_adam_softmax_solution_perslice(kernel_wt1, Bscrna,ct_identity,
