...
par_dict = {'N':[50]}#25,  75,  125, 150, 175]}# [ 5, 10, 50,  100,  200]
            #'nswaps_nbd':[2, 5, 10, 20, 50, 100]}
...
def cell2location_run(nneigh, inf_aver, **kwargs):
    ...
    for entry3 in real_samples: # iterate over real input samples or slices  
        ...
        for entry0 in [0]: # range(num_tissueslices+1): # iterate over simulated tissue slices for each input slice
            ...
            deconv_st1[key_name][entry3][entry0].index = [entry_idx.replace('meanscell_abundance_w_sf_','') for entry_idx in list(deconv_st1[key_name][entry3][entry0].index)]
