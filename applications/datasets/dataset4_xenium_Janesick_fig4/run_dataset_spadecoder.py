...
def spadecoder_run( Bbulk,ct_identity_bulk,ct_props,adata_sc_df,ct_identity_sc, mode=par_base['mode'],**kwargs):
    ...
    ################################## transcriptomic variable kernel ###############################################################################   
        ...
        if os.path.exists(kernel_path):
            ...
            # construct it 
            kernel_all, geary_metric = get_variabletranscr_bw(slice_order, slice_list,var_nspaneigh_transcr, mode, fixed_bw = bandwidth, Palign_dis=Palign_dis, min_wt=min_wt,method=align_tool, batch_sz=batch_sz)
    ...
    if mode == 'multislice':
        ...
        for tmp_slice_tgt in range(len(slice_order)):
            ...
            # if tmp_slice_tgt == 0:  # nothing before 
            #     slice_list_extra[tmp_slice_tgt] = slice_order[tmp_slice_tgt]
            ...
            if tmp_slice_tgt != 0: # not the first slice 
                ...
                kernel_wt_extra[tmp_slice_tgt].append(torch.from_numpy(kernel_all[ str(slice_order[tmp_slice_src]) + '_' + str(slice_order[tmp_slice_tgt])  ]).to(device))
            ...
            # mask = adj_wts_dict[tmp_slice_tgt] != 0.0
            # adj_wts_dict[tmp_slice_tgt] = adj_wts_dict[tmp_slice_tgt][mask]
            
            # slice_list_extra[tmp_slice_tgt] =  [t for t, m in zip(slice_list_extra[tmp_slice_tgt], mask) if m]
            # kernel_wt_extra[tmp_slice_tgt] =  [t for t, m in zip(kernel_wt_extra[tmp_slice_tgt], mask) if m]

            # slice_idx = len(slice_list_curr) - 1

            
            # with open('kernelvals', 'wb') as handle:
            #     pickle.dump(kernel_wt_list, handle, protocol=pickle.HIGHEST_PROTOCOL)
            
            # with open('slicevals', 'wb') as handle:
            #     pickle.dump(slice_list_curr, handle, protocol=pickle.HIGHEST_PROTOCOL)

            # original_indices = torch.nonzero(mask).squeeze()
            # print(slice_idx)
            # slice_idx = torch.where(original_indices == slice_idx)[0].item()
            # print(len(mask), len(slice_list_curr),slice_idx)
            ###########################################################################################################################################
    ...
    for currslice_idx in range(len(slice_order)): # iterate over real input samples or slices  
        ...
        # print(slice_list_curr)
        # print(slice_idx)
        # print(Bbulk)
        # print(ct_identity_bulk)
        # print(kernel3d_bw_slices)
        # print(adj_wts_curr)
        # print(kernel_wt_list[-1])
        # print((kernel_wt_list[-1]>0).sum(axis=0).max()) # shape)
        # sys.exit()
    ...
    write_slice1 = par_base['simdir'] + 'res1_'  +  mode + '_' + mode_nbd + '_sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) +  partesting_str + result_ext +  align_tool +  '_sc.pickle'
    with open(write_slice1, 'wb') as handle:
            pickle.dump(res1_full_sc, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
    write_slice1 = par_base['simdir'] + 'res1_'  + mode + '_' + mode_nbd + '_sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) +  partesting_str + result_ext +  align_tool + '_cluster.pickle'
    with open(write_slice1, 'wb') as handle:
            pickle.dump(res1_full_bulk, handle, protocol=pickle.HIGHEST_PROTOCOL)

    
    write_slice1 = par_base['simdir'] + 'alpha_'  +  mode + '_' + mode_nbd + '_sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) +  partesting_str + result_ext +  align_tool +  '_sc.pickle'
    with open(write_slice1, 'wb') as handle:
            pickle.dump(alpha_full_sc, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
    write_slice1 = par_base['simdir'] + 'alpha_'  + mode + '_' + mode_nbd + '_sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) +  partesting_str + result_ext +  align_tool + '_cluster.pickle'
    with open(write_slice1, 'wb') as handle:
