...
par_base = {###########. general path parameters ############################
            'adata_scrna_path':"../data/scrna_ref_norm1knolog.h5ad", # scRNA
            ...
            'num_augment':20,
            'do_augment':True,
            'batch_sz_spa':100, # default is 500
            'adj_wt_cutoff':1e-6,
            'max_spots_before_clipping':5000
            }
...
def spadecoder_run( Bbulk,ct_identity_bulk,ct_props,adata_sc_df,ct_identity_sc, mode=par_base['mode'],**kwargs):
    ...
    batch_sz_spa = kwargs.get('batch_sz_spa', par_base['batch_sz_spa'])
    adj_wt_cutoff = kwargs.get('adj_wt_cutoff', par_base['adj_wt_cutoff'])
    max_spots_before_clipping = kwargs.get('max_spots_before_clipping', par_base['max_spots_before_clipping'])

    partesting_str =     '_partesting_' + 'parlambda_' + str(par_lambda) + '_parbw_' + str(bandwidth) + '_nbdtype_' + mode_nbd + '_gtalign_' + str(gt_align) + '_' + str(augment) + '_kernel3d_bw_slices_' + str(kernel3d_bw_slices) + '_num_augment_' + str(num_augment) # + '_pareta1_' + str(par_eta1) +  '_3Dkernelbw_' + str(kernel3d_bw_slices_curr)
    ...
    for realslice in list(adata_spa[key_name][0].keys()):
        ...
        if (not gt_align) and  (mode == 'multislice'): # need to align
            ...
            align_path =  par_base['pickle_path'] + align_tool + '_align_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) +  suffix +  '_sample_' + realslice +  '_sim.pickle' # moscot_align_sptsz_50_nneigh_10_nbdswaps_2_normnolog_nov2024_1k.pickle' # moscot_me.pickle' #moscot_me.pickle' #par_base['pickle_path'] + align_tool + '_align_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) +  suffix + '.pickle'
            align_orig_path =  par_base['pickle_path'] + align_tool + '_alignorig_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) +  suffix +  '_sample_' + realslice +  '_sim.pickle'
            ...
            for slice1 in range(len(slice_list)):
                for slice2 in range(len(slice_list)):
                    if slice1 != slice2:
                        ...
                        Palign_dis[(str(slice1),str(slice2))] = kernel_wt1.copy()
            Palign_orig = Palign_dis.copy() 
        ...
        kernel_path = par_base['pickle_path'] + '/kernel_wt_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd)  + '_parbw_' + str(bandwidth) + suffix + '_' + align_tool + '_' + mode + '_' + mode_nbd + '_gtalign_' + str(gt_align) +  '_sample_' + realslice + '_sim.pickle'
        geary_path = par_base['pickle_path'] + '/geary_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd)  + '_parbw_' + str(bandwidth) + suffix + '_' + align_tool + '_' + mode + '_' + mode_nbd + '_gtalign_' + str(gt_align) +  '_sample_' + realslice +  '_sim.pickle'
        ...
        if os.path.exists(kernel_path):
            ...
            if mode_nbd == 'variable':
                ...
                # construct it 
                kernel_all, geary_metric = get_variable_bw(slice_order, slice_list,var_nspaneigh,var_bw, mode, Palign_dis=Palign_dis, min_wt=min_wt,method=align_tool, gt_align=gt_align)
        ...
        ################################## transcriptomic variable kernel ###############################################################################   
                ...
                # construct it 
                kernel_all, geary_metric = get_variabletranscr_bw(slice_order, slice_list,var_nspaneigh_transcr, mode, fixed_bw = bandwidth, Palign_dis=Palign_dis, min_wt=min_wt,method=align_tool, gt_align=gt_align, batch_sz=1)
        ...
        ################################## fixed kernel ###############################################################################      
                ...
                # construct it 
                kernel_all = get_fixed_bw(slice_order, slice_list, mode,  fixed_bw=bandwidth, fixed_nn=n_spatial_neigh, Palign_dis=Palign_dis, min_wt=min_wt,method=align_tool,gt_align=gt_align)
        ...
        if augment: 
            if mode == 'multislice':
                ...
                augment_path =  par_base['pickle_path'] + '/impute_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) + suffix + '_align_' + align_tool + '_gtalign_' + str(gt_align) + '_sample_' + realslice + '_numaugment_' + str(num_augment) + '_sim.pickle' 
                ...
                for tmp_slice_tgt in range(len(slice_order)):
                    ...
                    if tmp_slice_tgt != 0: # not the first slice 
                        ...
                        kernel_wt_extra[tmp_slice_tgt].append(kernel_all[ str(slice_order[tmp_slice_src]) + '_' + str(slice_order[tmp_slice_tgt])  ])
                        ...
                        for tmp_sim in range(0,num_augment):
                            ...
                            kernel_wt_extra[tmp_slice_tgt].append(np.eye(kernel_all[ str(tmp_slice_tgt) + '_' + str(tmp_slice_tgt)  ].shape[0]))
                    # add current slice 
                    ...
                    kernel_wt_extra[tmp_slice_tgt].append(kernel_all[ str(tmp_slice_tgt) + '_' + str(tmp_slice_tgt)  ])
                    ...
                    if tmp_slice_tgt != (len(slice_list) - 1): # not the first slice 
                        ...
                        for tmp_sim in range(0,num_augment):
                            ...
                            # kernel_wt_extra[realslice][tmp_slice_tgt].append(torch.eye(kernel_all[realslice][ str(slice_order[tmp_slice_tgt]) + '_' + str(slice_order[tmp_slice_tgt])  ].shape[0]).to(device)) #torch.from_numpy(kernel_all[ str(tgt_slice) + '_' + str(tgt_slice)  ]).to(device))
                            kernel_wt_extra[tmp_slice_tgt].append(np.eye(kernel_all[ str(slice_order[tmp_slice_tgt]) + '_' + str(slice_order[tmp_slice_tgt])  ].shape[0]))
            ...
            for tmp_slice_tgt in range(len(slice_order)):
                slice_list_extra[tmp_slice_tgt] = []
                adj_wts_dict[tmp_slice_tgt] = []
                kernel_wt_extra[tmp_slice_tgt] = []
                if tmp_slice_tgt != 0:
                    slice_list_extra[tmp_slice_tgt] = [slice_list[tmp_slice_tgt-1].copy()]
                    ...
                    kernel_wt_extra[tmp_slice_tgt].append(kernel_all[ str(tmp_slice_tgt-1) + '_' + str(tmp_slice_tgt)  ])
                slice_list_extra[tmp_slice_tgt].append(slice_list[tmp_slice_tgt].copy())
                # kernel_wt_extra[realslice][tmp_slice_tgt].append(torch.from_numpy(kernel_all[realslice][ str(tmp_slice_tgt) + '_' + str(tmp_slice_tgt)  ]).to(device))
                kernel_wt_extra[tmp_slice_tgt].append(kernel_all[ str(tmp_slice_tgt) + '_' + str(tmp_slice_tgt)  ])
                if tmp_slice_tgt != (len(slice_list)-1):
                    slice_list_extra[tmp_slice_tgt].append(slice_list[tmp_slice_tgt+1].copy())
                    # kernel_wt_extra[realslice][tmp_slice_tgt].append(torch.from_numpy(kernel_all[realslice][ str(tmp_slice_tgt+1) + '_' + str(tmp_slice_tgt)  ]).to(device))
                    kernel_wt_extra[tmp_slice_tgt].append(kernel_all[ str(tmp_slice_tgt+1) + '_' + str(tmp_slice_tgt)  ])
        ...
        # write adj weights to file
        if mode == 'multislice':
            adj_wt_path = par_base['pickle_path'] + '/adj_wts_'  + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd) + '_kernel3Dbw_' + str(kernel3d_bw_slices) + mode + '_' + mode_nbd +   partesting_str + result_ext +  align_tool +  '_gtalign_' + str(gt_align) + '_sample_' + realslice + '_adjwts.pickle'
        ...
        for currslice_idx in range(len(slice_order)): # iterate over real input samples or slices  
            ...
            if mode != 'multislice':
                ...
                slice_idx = 0 
                adj_wts_curr = None
                kernel_wt_list = [torch.from_numpy(kernel_all[ str(slice_order[currslice_idx]) + '_' + str(slice_order[currslice_idx])  ]).to(device)]
            else:
                ...
                # doing this way to avoid memory issues for large numebr of spots and augmented slices 
                # if number of spots too large, clip augmented slices, based on adj_wts
                if slice_list_curr[slice_idx].shape[0] > max_spots_before_clipping:
                    print("clipping augmented slices based on adj_wts")
                    truncated_adj_wts_curr = [a for a in adj_wts_curr if a >= adj_wt_cutoff]
                    kernel_wt_list = [torch.from_numpy(b).to(device) for a, b in zip(adj_wts_curr, kernel_wt_extra[currslice_idx]) if a >= adj_wt_cutoff]
                    slice_list_curr = [c for a, c in zip(adj_wts_curr, slice_list_curr) if a >= adj_wt_cutoff]
                    adj_wts_curr   = truncated_adj_wts_curr     
                    # slice index will be the one with maximum weight 
                    slice_idx = adj_wts_curr.index(max(adj_wts_curr))
                else: # make tensor
                    kernel_wt_list = [torch.from_numpy(kernel_wt_extra[currslice_idx][tmp_idx]).to(device) for tmp_idx in range(len(kernel_wt_extra[currslice_idx]))]
    ...
    #         if not os.path.exists(align_orig_path):
    #             with open(align_orig_path, 'wb') as handle:
    #                 pickle.dump(Palign_orig, handle, protocol=pickle.HIGHEST_PROTOCOL)
        ...
        for modeNbd_curr in par_to_iterate['modeNbd']: # fixed, variable, variabletranscr
            if mode_curr == 'multislice':
                for align_tool_curr in par_to_iterate['align_tool']:
                    ...
                    for N_curr in par_to_iterate['N']:
                        spadecoder_run(  Bbulk,ct_identity_bulk,ct_props,adata_scrna.to_df().T,ct_identity_sc,mode=mode_curr,**{'modeNbd':modeNbd_curr, 'align_tool':align_tool_curr, 'N':N_curr})# par_tune:par_value}) 
            else:
                # for par_tune in par_to_iterate.keys():
                #         if par_tune not in ['mode','modeNbd','align_tool']:
                #             for par_value in par_to_iterate[par_tune]:
                ...
                for N_curr in par_to_iterate['N']:
                    spadecoder_run(  Bbulk,ct_identity_bulk,ct_props,adata_scrna.to_df().T,ct_identity_sc,mode=mode_curr,**{'modeNbd':modeNbd_curr, 'align_tool':'moscot', 'N':N_curr})# par_tune:par_value}) 
