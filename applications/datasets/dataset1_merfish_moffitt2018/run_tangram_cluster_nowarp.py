...
# adata_scrna_path = '../data/final_filtered_normcorrected.h5ad' #'/Users/macrinalobo/Documents/zhanglab/2024AprSpadecoder/dataset1_merfish_moffitt2018_50/data/final_filtered.h5ad'
adata_scrna_path = "../data/scrna_ref_normnolog_nov2024_1k.h5ad"
# types_of_spots = ['linear']
# num_simulations = 1 # 3
num_tissueslices = 10 # 10

result_metric = ['orig_rmse',  'ari','avg_corr_pe','avg_jsd'] # ['orig_rmse', 'new_rmse', 'ari', 'purity','avg_corr_sp','avg_corr_pe','avg_jsd']
...
par_dict = {'N':[25,  75,  125, 150, 175]}# [ 5, 10, 50,  100,  200]
            #'nswaps_nbd':[2, 5, 10, 20, 50, 100]}
...
def tangram_run(nneigh, adata_sc, **kwargs):
    ...
    for entry3 in real_samples: # iterate over real input samples or slices  
        ...
        for entry0 in range(num_tissueslices+1): # iterate over simulated tissue slices for each input slice
            ...
            ad_map = tg.map_cells_to_space(
                adata_sc,
                adata_spot,
                mode='clusters',verbose=False,  device="cuda:0", # device="cpu",
            ...
            deconv_st1[key_name][entry3][entry0] = celltype_density.T # [adata_spot_swap[key_name][entry0][entry3].obs.columns].T
    ...
    write_slice1 = simdir + 'deconv_st1_'  + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd_curr) + '_tangram.pickle'
    with open(write_slice1, 'wb') as handle:
            pickle.dump(deconv_st1, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
    metrics_slice1  = simdir + 'metrics_st1_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd_curr) + '_tangram.pickle'
    with open(metrics_slice1, 'wb') as handle:
            pickle.dump(results_df, handle, protocol=pickle.HIGHEST_PROTOCOL)

    metrics_slice1  = simdir + 'runtimes_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd_curr) + '_tangram.csv'
...
if __name__ == "__main__":
    ...
    # for nswaps_nbd in nswaps_nbd_range:
    ...
    #     args.append((N,  nneigh, nswaps_nbd, adata_sc))
    ...
    #     # print(name)
    #     sema.acquire()
    #     proc = Process(target=tangram_run, args=arg)
    #     procs.append(proc)
    #     proc.start()
