...
# N_range = [ 5, 10, 50,  100,  200],
# nswaps_nbd_range = [2, 5, 10, 20, 50, 100]

par_dict = {'N':[50]}# [ 5, 10, 50,  100,  200] 25,  75,  125, 150, 175
...
def tangram_run( nneigh, adata_sc, **kwargs):
    ...
    for entry3 in real_samples: # iterate over real input samples or slices  
        ...
        cell_wts[key_name][entry3] = {} 
        
        for entry0 in [0]:# range(num_tissueslices+1): # iterate over simulated tissue slices for each input slice
            ...
            cell_wts[key_name][entry3][entry0] =  ad_map.to_df()
    ...
    metrics_slice1  = simdir + 'cellwts_' + 'sptsz_' + str(N_curr)  + '_nneigh_' + str(nneigh) + '_nbdswaps_' + str(nswaps_nbd_curr) + '_tangramsc.pickle'
    with open(metrics_slice1, 'wb') as handle:
